...
namespace WSIMap
{
    public class OpenGlControl : UserControl
    {
        ...
        /// </summary>
        public bool InitializeContexts()
        {
            ...
            {                              // Did we not get a device context?
                MessageBox.Show("Cannot create a GL device context.", "Fatal Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            ...
            {                                          // Did windows not find a matching pixel format?
                MessageBox.Show("Cannot find a suitable PixelFormat.", "Fatal Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }

            if (!Gdi.SetPixelFormat(deviceContext, pixelFormat, ref pfd))
            {  // Are we not able to set the pixel format?
                MessageBox.Show("Cannot set the chosen PixelFormat.  Chosen PixelFormat was " + pixelFormat + ".", "Fatal Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            ...
            {                           // Are we not able to get a rendering context?
                MessageBox.Show("Cannot create a GL rendering context.", "Fatal Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }

            if (!Wgl.wglMakeCurrent(deviceContext, renderingContext))
            {
                MessageBox.Show("Cannot activate the GL rendering context.", "Fatal Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        ...
        public void MakeCurrent()
        {
            ...
            if (!Wgl.wglMakeCurrent(deviceContext, renderingContext))
            {
                //if (!EventLog.SourceExists("WSIMap"))
                //    EventLog.CreateEventSource("WSIMap", "Application");
                //EventLog.WriteEntry("WSIMap", "wglMakeCurrent failed");
                return;
            }
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
			...
			Feature.ConfirmMainThread("OpenGlControl OnPaint()");
#endif
