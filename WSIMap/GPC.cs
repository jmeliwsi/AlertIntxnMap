namespace GPC
{
    ...
    {
        /// <summary>
        /// Takes a collection of WSIMap.Polygons and returns a combined list any polygons that overlap without holes.
        /// Limitations: If the combined polygon contains 'holes', the method will return original polygon list since WSIMap.Polygon cannot handle 'holes'.
        /// Polygons that cross the Prime Meridian are returned individually (not combined) because the combine code does not handle this currently.
        /// Polygons need to share colors, opacity, and line width.
        /// Polygon borders may not look correct because of the way the combining handles the PM, so if the polygons are expected to cross the PM a border is not recommended.
        /// </summary>
        /// <param name="polygonList">List of polygons to combine</param>
        /// <param name="borderColor"></param>
        /// <param name="borderWidth"></param>
        /// <param name="fillColor"></param>
        /// <param name="opacity"></param>
        /// <returns>A List of WSIMap.Polygons</returns>
         public static List<WSIMap.Polygon> CombinePolygonList(List<WSIMap.Polygon> polygonList, Color borderColor, uint borderWidth, Color fillColor, uint opacity)
         {
             List<WSIMap.Polygon> combinedList = new List<WSIMap.Polygon>();

             //The GPC library does not handle crossing the dateline or the Prime meridian well,
             //So we will combine polygons on either side of the PM into two separate collections
             List<Polygon> gpcRightOfPMPolygonList = new List<Polygon>();
             List<Polygon> gpcLeftOfPMPolygonList = new List<Polygon>();
             List<Polygon> crossesPrimeMeridian = new List<Polygon>();

             //convert all WSIMap.Polygons into GPC Polygons
             foreach(WSIMap.Polygon p in polygonList)
             {
                 Polygon newPoly = new Polygon(new VertexList(p.PointList.ToArray()), false);

                 //We need to know these four things to decide which collection to put the polygon in.
                 bool onAntiMeridian = false;
                 bool toLeftOfPrimeMeridan = false;
                 bool onPrimeMeridian = false;
                 bool toRightOfPrimeMeridan = false;

                 //Iterate through all points in the point list, setting values as we go
                 //At the end of this loop, we will have enough data to know which collection to use.
                 for(int i = 0; i < p.Count; i++)
                 {
                     if (p.PointList[i].X == -180.0 || p.PointList[i].X == 180.0)
                         onAntiMeridian = true;

                     else if (p.PointList[i].X == -0.0 || p.PointList[i].X == 0.0)
                         onPrimeMeridian = true;

                     else if (p.PointList[i].X > 0.0)
                         toLeftOfPrimeMeridan = true;

                     else if (p.PointList[i].X < 0.0)
                         toRightOfPrimeMeridan = true;
                 }

                 if(onPrimeMeridian && toLeftOfPrimeMeridan)
                     gpcLeftOfPMPolygonList.Add(newPoly);

                 if (onPrimeMeridian && !toLeftOfPrimeMeridan)
                     gpcRightOfPMPolygonList.Add(newPoly);

                 if (onAntiMeridian && toLeftOfPrimeMeridan)
                     gpcLeftOfPMPolygonList.Add(newPoly);

                 if (onAntiMeridian && !toLeftOfPrimeMeridan)
                     gpcRightOfPMPolygonList.Add(newPoly);

                 if(!onPrimeMeridian && !onAntiMeridian && toLeftOfPrimeMeridan && !toRightOfPrimeMeridan)
                     gpcLeftOfPMPolygonList.Add(newPoly);

                 if (!onPrimeMeridian && !onAntiMeridian && toRightOfPrimeMeridan && !toLeftOfPrimeMeridan)
                     gpcRightOfPMPolygonList.Add(newPoly);

                 //These are not handled by the GPC library and we should return them as individual polygons.
                 if(toLeftOfPrimeMeridan && toRightOfPrimeMeridan)
                     crossesPrimeMeridian.Add(newPoly);
             }
             Polygon combinedRightOfPMPoly = new Polygon();
             Polygon combinedLeftOfPMPoly = new Polygon();
             try
             {
                 for (int i = 0; i < gpcRightOfPMPolygonList.Count; i++)
                 {
                     if (i == 0)
                         combinedRightOfPMPoly = gpcRightOfPMPolygonList[0];
                     else
                         combinedRightOfPMPoly = GpcWrapper.Clip(GpcOperation.Union, combinedRightOfPMPoly, gpcRightOfPMPolygonList[i]);
                 }

                 for (int i = 0; i < gpcLeftOfPMPolygonList.Count; i++)
                 {
                     if (i == 0)
                         combinedLeftOfPMPoly = gpcLeftOfPMPolygonList[0];
                     else
                         combinedLeftOfPMPoly = GpcWrapper.Clip(GpcOperation.Union, combinedLeftOfPMPoly, gpcLeftOfPMPolygonList[i]);
                 }

                 //Fusion does not currently support Polygons with 'holes', so just return the original polygon list. This may be possible to determine earlier.
                 if (combinedLeftOfPMPoly.ContourIsHole.Any(x => x) || combinedRightOfPMPoly.ContourIsHole.Any(x => x))
                     return polygonList;
             }
             catch(Exception e)
             {
                 //If something has gone wrong, we just return the original polygon list and it should be a recoverable error.
                 return polygonList;
             }

             //Split the combined polygons in the Contour collections and create WSIMap.Polygons to return
             for (int i = 0; i < combinedRightOfPMPoly.Contour.Length; i++)
                 combinedList.Add(new WSIMap.Polygon(new List<PointD>(combinedRightOfPMPoly.Contour[i].ToPointsD()), borderColor, borderWidth, fillColor, opacity));

             for (int i = 0; i < combinedLeftOfPMPoly.Contour.Length; i++)
                 combinedList.Add(new WSIMap.Polygon(new List<PointD>(combinedLeftOfPMPoly.Contour[i].ToPointsD()), borderColor, borderWidth, fillColor, opacity));

             //Add any original polygons that cross the PM to the collection
             foreach (Polygon p in crossesPrimeMeridian)
                 if(p.Contour.Count() > 0)
                    combinedList.Add(new WSIMap.Polygon(new List<PointD>(p.Contour[0].ToPointsD()), borderColor, borderWidth, fillColor, opacity));
    ...
    public class VertexList
    {
        ...
        public PointD[] ToPointsD()
        {
            PointD[] vertexArray = new PointD[NofVertices];
            for (int i = 0; i < NofVertices; i++)
            {
                vertexArray[i] = new PointD((double)Vertex[i].X, (double)Vertex[i].Y);
            }
            return vertexArray;
        }
        ...
        public ArrayList ToArrayList()
        {
            ...
            for (int i = 0; i < NofVertices; i++)
            {
                vertexList.Add(new PointD(Vertex[i].X, Vertex[i].Y));
            }
            return vertexList;
        }
    ...
    public class Polygon
    {
        ...
        {
            AddContour(contour, contourIsHole);
        }
